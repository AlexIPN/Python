...
class Configuration():
	...
	def __init__(self):
		try:
			...
			CONSUMER_KEY = config.get('Twitter OAuth', 'gGZgkwKOG6qzXKtz0qMbDDifZ')
			CONSUMER_SECRET = config.get('Twitter OAuth', 'm11zB6jZLoRJ9lArwW67mvyK6IQ0iesGMk7OkE5YmSQ1yF2qt2')
			ACCESS_TOKEN = config.get('Twitter OAuth', '874427357834039297-KhUj3U9uZFTOLlm0iQoyzjHlw5c0NZg')
			ACCESS_TOKEN_SECRET = config.get('Twitter OAuth', 'xXWSbjllHbpAqzicff795URg9EhExnRKhCCyi8OieatY7') 
		...
		try:
			...
			for m in medias:
				...
				if socialnetwork.lower().find("instagram") >= 0:
					#: Instagram
					#: ----------------------------------------------
					urls = re.search(';" href="/(.*)/" data-reactid="7"><img class="', html)
				
					if urls:
						username = urls.group(1)

					urls = re.search('<meta property="og:title" content="Instagram photo by (.*) •', html)

					if urls:
						name = urls.group(1)
						
					urls = re.search('<img class="_a012k" src="(.*)" data-reactid="8"', html)
...
class User_Tweets:
	...
	def set_find_information(self, text, tweet):
		try:
			...
			if add:
				self.tweets_find.append([tweet.text, tweet.created_at.strftime('%m/%d/%Y'), tweet.created_at.time(), tweet.id, screen_name, profile_image_url, location, name])
...
# ----------------------------------------------------------------------
def generates_HTML_file(parameters, user, source, social, hashtag, mention, geolocation, user_images, user_tweets, search, user_conversations):
	...
	try:
		...
		if not args.username and not args.latlonkm:
			...
			if args.latlonkm:
				...
				search.set_geolocation_information(parameters.api, parameters.find, parameters.search, args.tweets, parameters.sdate, parameters.edate, parameters.stime, parameters.etime, hashtag, mentions, social_networks, parameters, user_images)
				print "\n\t\tOK"
					
			else:								
				print "\tLooking info for @"  + args.username + ":\n"
				
				if not args.information and not args.file and not args.number and not args.d and not args.sources and not args.hashtags and not args.mentions and not args.text and not args.meta and not args.conv and not args.socialnetworks: 
					...
					if args.sources or args.hashtags or args.mentions or args.d or args.file or args.number or args.text or args.meta or args.conv or args.socialnetworks:
						page = 1
						tweets_count = 0	
						print "\n\t\tExecuting operations..."
						...
						if args.meta: 
							...
							user_images.get_metadata(user.profile_image_url.replace("_normal.", "."), 1, user.screen_name)
			...
			if args.hashtags or args.latlonkm:
				# Get global ifnromatino about hashtags included in tweets
			...
			if args.latlonkm or args.information or args.file or args.number or args.d or args.sources or args.hashtags or args.mentions or args.text or args.meta or args.conv or args.socialnetworks:
				print "\n\t\tGenerating report..."
				generates_HTML_file(parameters, user, source, social_networks, hashtag, mentions, geolocation, user_images, user_tweets, search, user_conversations)
				if os.name == "nt":
...
def get_tagged_users_from_instagram(html):
	...
	try:
		...
		urls = re.search('"owner": {(.*), "profile_pic_url": "(.*)", "full_name": "(.*)", "', html)

		if urls:
			profile_image = str(urls.group(2)) + ".jpg"

		urls = re.findall(' "user": {"username": "(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+"}', html)  
		for users in urls:
			tagged_users.append(users[23:len(users)-2])
		
		urls = re.search(', "owner": {"username": "(.*)", "is_unpublished', html)
		if urls:
			owner = urls.group(1)
...
def get_url_media_from_foursquare(url):
	...
	try:
		...
		urls = re.search('","canonicalUrl":"https:(.*)"venue":{"', html)
...
def get_url_facebook_from_foursquare(url):
	...
	try:
		...
		try:
			...
			return url_facebook, user_facebook 
...
def main():
	...
	try:
		...
		parser.add_argument('--conv', action='store_true', dest='conv', help='show user conversations')
		parser.add_argument('--sdate', dest='sdate', help='filter the results for this start date (format: yyyy-mm-dd)')
		parser.add_argument('--edate', dest='edate', help='filter the results for this end date (format: yyyy-mm-dd)')
		parser.add_argument('--stime', default='00:00:00', dest='stime', help='filter the results for this start time (format: HH:MM:SS)')
		parser.add_argument('--etime', default='23:59:59', dest='etime', help='filter the results for this end date (format: HH:MM:SS)')
		...
		parser.add_argument('--social', action='store_true', dest='socialnetworks', default='', help='Identify social networks identities')
		...
		parser.add_argument('--search', dest='latlonkm', default='', help='search tweets in LATitude, LONgitude, and KMs (distance)')
